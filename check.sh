...
check_readme() {
	...
	if (( $(echo "$total == 0" |bc -l)  )); then
		echo "$README points unawarded. Points for the tests are $total!"
	elif [ ! -f $README ]; then
		echo "$README lipsa."
		...
		score=$max_score
		echo "$README OK. Final grade will be awarded during the checking."
	fi
...
check_coding_style_dummy() {
	...
	total=$(bc <<< "$total + $score")
	echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
	echo "Final grade will be awarded during the checking (including valgrind testing)."
	echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
check_coding_style_full() {
	...
	if (( $(echo "$total == 0" |bc -l)  )); then
		score=0
		echo "Coding style points unawarded. Points for the tests are $total!"
	else
		...
		if [ $cnt_cpp -gt 0 ] || [ $cnt_java -gt 0 ]; then
			...
			echo "Coding style OK. Final grade can be modified during the checking."
		fi
